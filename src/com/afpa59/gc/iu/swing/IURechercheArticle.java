...
public class IURechercheArticle extends JFrame{
	 ...
	 * @param h
	 */
	public IURechercheArticle(String titre, int w, int h, Mode mode){
		super(titre);
	...
	private JPanel getTitlePanel() {
		...
		JLabel titre = new JLabel(" Rechercher un article ");
	...
	private JPanel getMainPanel() {
		...
		labelLibelle = new JLabel("Rechercher par nom : ");
	...
	public void actionRecherche(ActionEvent event) throws ObjetInexistantException{
		...
		String getNom = txtLibelle.getText();
		...
		if(getId.equals("") && getNom.equals("")){ //les 2 champs sont vides
			...
			if(getId.matches("^\\d+$")){ //si l'id est bien un entier
				...
				if(mode==Mode.SUPPRESSION){
					...
					new IUAffichageArticle("Affichage d'un article", 400, 400, this.mode, article);
				}
				this.dispose();
			}else{
			...
			List<Entite> articles = ServiceArticle.getInstance().rechercherParLibelle(getNom);
			this.dispose();
			new IUAffichageListeArticle("Résultat de la recherche", 500, 500, this.mode, articles);
		}
